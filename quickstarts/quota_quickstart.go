package quickstarts

import (
	console "github.com/openshift/api/console/v1"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
)

const QuotaQuickStartName = "create-cluster-template-quota"

func getQuotaTasks() []console.ConsoleQuickStartTask {
	task1 := console.ConsoleQuickStartTask{
		Title: "Create a new quota.",
		Description: `1. From the **All Clusters** perspective, navigate to the [Cluster Templates](k8s/cluster/clustertemplate.openshift.io~v1alpha1~ClusterTemplate) page by clicking **Infrastructure > Cluster templates**.
2. Click the **Quota management** tab and then the **Create a quota** button.
3. Select a namespace on which the following limits will be applied.
4. (optional) Add a consumption limit.
5. Select one or more templates that will be limited and associated with this quota.
6. (optional) Add restrictions on the selected templates.
7. Click **Create**.`,
		Review: &console.ConsoleQuickStartTaskReview{
			FailedTaskHelp: "This task isn’t verified yet. Try the task again.",
			Instructions: `Verify that the new quota added.
1. Click the **Quota management** tab.
2. Do you see the new quota in the table?`,
		},
		Summary: &console.ConsoleQuickStartTaskSummary{
			Failed:  "Try the steps again.",
			Success: "You successfuly created a quota.",
		},
	}

	return []console.ConsoleQuickStartTask{task1}
}

func GetQuotaQuickStart() *console.ConsoleQuickStart {
	tasks := getQuotaTasks()
	return &console.ConsoleQuickStart{
		ObjectMeta: metav1.ObjectMeta{
			Name: "create-cluster-template-quota",
		},
		Spec: console.ConsoleQuickStartSpec{
			Conclusion:      "The created quota now limits users when creating cluster from the selected templates in the selected namespace.",
			Description:     "Control cluster template consumption by limiting the use of them to specific namespaces.",
			DisplayName:     "Limit cluster template access using quotas",
			Introduction:    "By creating quotas, you’ll be able to manage which exact templates can be used in a given namespaces, and how many instances of these templates can be created.",
			Icon:            `data:image/svg+xml;base64,`,
			DurationMinutes: 5,
			Tasks:           tasks,
		},
	}
}
